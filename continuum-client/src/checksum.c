...
/* CRC8 lookup table - polynomial x^8 + x^5 + x^4 + 1 */
...
/* CRC32 reflected table (polynomial 0xEDB88320) */
...
u32 crc32_map(const u8* tiles, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        u8 tile = tiles[i];
        /* Skip empty tiles and certain tile types */
        if (tile != 0 && (tile < 0xA1 || tile == 0xAB)) {
            crc = (crc >> 8) ^ crc32_table[(crc ^ tile) & 0xFF];
        }
    }
    return crc;
}

/* VIE executable checksum - hardcoded constants from reverse engineering */
